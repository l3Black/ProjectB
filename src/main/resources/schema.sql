...
CREATE UNIQUE INDEX UX_doc_type_code ON DOCUMENT_TYPE (code);

CREATE TABLE IF NOT EXISTS COUNTRY
(
    id   INTEGER COMMENT 'Уникальный идентификатор' PRIMARY KEY,
    code INTEGER     NOT NULL COMMENT 'Код страны по ОКСМ',
...
(
    ...
    region      VARCHAR(50) NOT NULL COMMENT 'Название региона',
    ...
    home_number VARCHAR(30) COMMENT 'Номер дома, корпус, строение'
);
...
(
    ...
    version          INTEGER NOT NULL COMMENT 'Служебное поле hibernate',
    document_type_id INTEGER NOT NULL COMMENT 'Внешний ключ на справочник типов документов',
    date_issue       DATE COMMENT 'Дата выдачи документа'
);
...
(
    ...
    inn        BIGINT       NOT NULL COMMENT 'Индентификационный номер налогоплательщика',
...
CREATE UNIQUE INDEX UX_org_inn ON ORGANIZATION (inn);
...
(
    ...
    is_identified BOOLEAN      NOT NULL COMMENT 'Прошел ли пользователь идентификацию'
);
...
CREATE UNIQUE INDEX UX_user_doc ON USER (doc_number);
